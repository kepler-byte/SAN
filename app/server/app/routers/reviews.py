...
async def find_review_in_book(book: dict, review_id: str) -> Tuple[Optional[dict], Optional[int]]:
    """Find a review in a book and return the review and its index"""
...
async def update_book_rating(book_id: str) -> None:
    """Recalculate and update the average rating for a book"""
    book = await book_collection.find_one({"_id": ObjectId(book_id)})
    reviews = book.get("reviews", [])
...
def format_review_response(review: dict, current_username: str) -> ReviewResponse:
    ...
    return ReviewResponse(
        ...
        is_owner=(review.get("user_id") == current_username)
    )
...
async def create_review(
    ...
    try:
        ...
        if not await check_review_permission(book_id, current_user["username"]):
            raise HTTPException(
                status_code=403, 
        ...
        if await check_user_already_reviewed(book, current_user["username"]):
            raise HTTPException(
                status_code=400, 
        ...
        return format_review_response(new_review, current_user["username"])
...
async def get_book_reviews(
    ...
    try:
        ...
        # Format reviews
        formatted_reviews = [
            format_review_response(rev, current_user["username"]) 
...
async def update_review(
    ...
    try:
        ...
        # Find the review
        review_data, review_index = await find_review_in_book(book, review_id)
        ...
        # Get all reviews and update the specific one
        reviews = book.get("reviews", [])
        ...
        return format_review_response(review_data, current_user["username"])
...
async def delete_review(
    ...
    try:
        ...
        # Find the review
        review_data, _ = await find_review_in_book(book, review_id)
...
@router.get("/user/reviews", response_model=UserReviewsResponse)
            ...
            for review in book.get("reviews", []):
                if review.get("user_id") == current_user["username"]:
                    user_reviews.append({
                        ...
                        "review": format_review_response(review, current_user["username"])
                    })
